"""Roo Code program — mcp_settings.json, env-based API keys."""
import json, uuid
from pathlib import Path
from ..settings import ROO_CFG


class RooCodeProgram:
    id = "roo-code"
    name = "Roo Code"
    letter = "R"
    config_path = ROO_CFG
    ...
    def read_config(self) -> dict:
        if not ROO_CFG.exists(): return {}
        try: return json.loads(ROO_CFG.read_text())
        except: return {}

    def write_config(self, data: dict):
        ROO_CFG.parent.mkdir(parents=True, exist_ok=True)
        ROO_CFG.write_text(json.dumps(data, ensure_ascii=False, indent=2))
    ...
    def import_accounts(self, accounts, existing_keys, existing_urls):
        ...
        accounts.append({
            "id": uuid.uuid4().hex[:8], "name": f"Roo Code ({api_key[:8]}...)",
            "provider": "openai" if api_key.startswith("sk-") else "openai",
            "api_key": api_key, "base_url": base_url, "model": "",
            "source_path": str(ROO_CFG),
        })
        imported.append(f"Roo Code ({api_key[:8]}...)")
        return imported

    def apply_account(self, acc, current_config=None):
        return False, "roo-code: edit mcp_settings.json manually"
